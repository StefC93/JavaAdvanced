...
public class CallLogReport {
	...
	public List<CallLog> createArchiveAndSortByDate(Date archiveStartDate) {
		...
		return callLogList.stream().filter(callLog -> callLog.getDatum().before(archiveStartDate))
				// // incl. rapport 4
				// // merk op dat we hier in tegenstelling tot de Sets niet nog
				// eens moeten comparen op de unieke id
				.sorted(Comparator.comparing((CallLog callLog) -> callLog.getDatum())).collect(Collectors.toList());
	}
	...
	public void sortByPrioAndDate() {
		Set<CallLog> callLogSetAuto = new TreeSet<>(Comparator.comparingInt((CallLog cl) -> cl.getPrio())
				.thenComparing((CallLog cl) -> cl.getDatum()).thenComparingInt((CallLog cl) -> cl.getId()));
